...
class AdjustingTraceExporter {
  ...
  adjust(spans) {
    ...
    const rootSpan = spans.find(
      (s) => Object.keys(s.attributes).includes("genkit:isRoot")
    );
    if (rootSpan) {
      const rootSpanFailed = rootSpan.attributes["genkit:state"] === "error";
      const anotherFailedSpan = spans.find(
        (s) => !Object.keys(s.attributes).includes("genkit:isRoot") && s.attributes["genkit:state"] === "error"
      );
      if (rootSpanFailed && !anotherFailedSpan) {
        rootSpan.attributes["genkit:failedSpan"] = rootSpan.attributes["genkit:name"];
        rootSpan.attributes["genkit:failedPath"] = rootSpan.attributes["genkit:path"];
      }
    }
  ...
  tickTelemetry(span, paths) {
    ...
    if (isRoot) {
      import_feature.featuresTelemetry.tick(
        span,
        unused,
        this.logInputAndOutput,
        this.projectId
      );
      import_path.pathsTelemetry.tick(span, paths, this.logInputAndOutput, this.projectId);
      span.attributes["genkit:rootState"] = span.attributes["genkit:state"];
    }
    if (type === "action" && subtype === "model") {
      import_generate.generateTelemetry.tick(
        span,
        unused,
        this.logInputAndOutput,
        this.projectId
    ...
    if (type === "action" || type === "flow" || type == "flowStep") {
      import_action.actionTelemetry.tick(
        span,
        unused,
        this.logInputAndOutput,
        this.projectId
    ...
    if (type === "userEngagement") {
      import_engagement.engagementTelemetry.tick(
        span,
        unused,
        this.logInputAndOutput,
        this.projectId
  ...
  markFailedSpan(span) {
    if (span.attributes["genkit:state"] === "error" && !span.attributes["genkit:isRoot"]) {
